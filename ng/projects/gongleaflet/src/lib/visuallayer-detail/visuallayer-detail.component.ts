...
export class VisualLayerDetailComponent implements OnInit {
	...
	state: VisualLayerDetailComponentState = VisualLayerDetailComponentState.CREATE_INSTANCE
			...
			frontRepo => {
				...
				switch (this.state) {
					...
					case VisualLayerDetailComponentState.UPDATE_INSTANCE:
						let visuallayer = frontRepo.VisualLayers.get(this.id)
						console.assert(visuallayer != undefined, "missing visuallayer with id:" + this.id)
		...
		switch (this.state) {
			...
			default:
				this.visuallayerService.postVisualLayer(this.visuallayer).subscribe(visuallayer => {
					...
					this.visuallayer = new (VisualLayerDB) // reset fields
				});
		}
	}
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.visuallayer.ID!
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "VisualLayer"

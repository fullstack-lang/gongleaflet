...
// VisualIconDetailComponent is initilizaed from different routes
// VisualIconDetailComponentState detail different cases 
enum VisualIconDetailComponentState {
	CREATE_INSTANCE,
	UPDATE_INSTANCE,
...
export class VisualIconDetailComponent implements OnInit {
	...
	state: VisualIconDetailComponentState = VisualIconDetailComponentState.CREATE_INSTANCE
	...
	ngOnInit(): void {
		...
		if (this.id == 0) {
			this.state = VisualIconDetailComponentState.CREATE_INSTANCE
		} else {
			if (this.originStruct == undefined) {
				this.state = VisualIconDetailComponentState.UPDATE_INSTANCE
			} else {
	...
	getVisualIcon(): void {
		...
		this.frontRepoService.pull().subscribe(
			frontRepo => {
				...
				switch (this.state) {
					case VisualIconDetailComponentState.CREATE_INSTANCE:
						this.visualicon = new (VisualIconDB)
						break;
					case VisualIconDetailComponentState.UPDATE_INSTANCE:
						let visualicon = frontRepo.VisualIcons.get(this.id)
						console.assert(visualicon != undefined, "missing visualicon with id:" + this.id)
		...
		switch (this.state) {
			...
			default:
				this.visualiconService.postVisualIcon(this.visualicon).subscribe(visualicon => {
					...
					this.visualicon = new (VisualIconDB) // reset fields
				});
		}
	}
	...
	openReverseSelection(AssociatedStruct: string, reverseField: string, selectionMode: string,
		...
		if (selectionMode == SelectionMode.ONE_MANY_ASSOCIATION_MODE) {
			...
			dialogData.ID = this.visualicon.ID!
		...
		if (selectionMode == SelectionMode.MANY_MANY_ASSOCIATION_MODE) {
			...
			// set up the source
			dialogData.SourceStruct = "VisualIcon"
